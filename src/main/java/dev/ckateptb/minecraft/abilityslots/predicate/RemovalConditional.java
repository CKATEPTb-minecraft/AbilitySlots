...
public interface RemovalConditional extends Conditional<Ability> {
    ...
    class Builder {
        ...
        public Builder slot() {
            policies.add((user, ability) -> {
                AbilityDeclaration information = user.getSelectedAbility();
        ...
        public Builder costInterval(long interval) {
            ...
            policies.add((user, ability) -> {
                if(System.currentTimeMillis() > expiredEnergySafeTime.get()) {
                    ...
                    return !user.removeEnergy(ability.getDeclaration().getCost());
                }
                return false;
            });
            return this;
        ...
        public Builder water(boolean shouldStayInWater) {
            policies.add((user, ability) -> {
                boolean inWater = user.getLocation().toLocation(user.getWorld()).getBlock().getType() == Material.WATER;
        ...
        public Builder liquid(boolean shouldStayInLiquid) {
            policies.add((user, ability) -> {
                boolean inLiquid = user.getLocation().toLocation(user.getWorld()).getBlock().isLiquid();
        ...
        public Builder passable(boolean shouldStayInPassable) {
            policies.add((user, ability) -> {
                boolean passable = user.getLocation().toLocation(user.getWorld()).getBlock().isPassable();
